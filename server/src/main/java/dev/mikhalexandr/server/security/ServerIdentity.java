 ...
 *
 * <p>Создаётся при старте сервера через {@link VaultPkiClient}: приватный ключ генерится локально и
 * существует только в памяти процесса, сертификат получается от Vault через CSR-подпись
 */
