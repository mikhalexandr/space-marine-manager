...
import dev.mikhalexandr.server.exceptions.FileWriteException;
import java.io.*;
  ...
  private LinkedList<SpaceMarine> parseValidMarines(JsonArray array) {
    ...
    for (JsonElement element : array) {
      try {
        ...
        if (marine == null) {
          LOGGER.warn("Невалидный объект пропущен: элемент равен null");
          continue;
        }
        marine.validate();
        if (!usedIds.add(marine.getId())) {
          LOGGER.warn("Невалидный объект пропущен: дубликат id={}", marine.getId());
          continue;
        }
        result.add(marine);
      } catch (ValidationException | JsonParseException | IllegalStateException e) {
        LOGGER.warn("Невалидный объект пропущен: {}", e.getMessage());
      }
    }
