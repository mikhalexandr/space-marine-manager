 ...
 * учётные данные сохраняются в {@link TcpClient} и прикрепляются к запросам потом
 */
public final class AuthGateway {
  ...
  /**
   * Запрашивает действие, пока пользователь не войдёт, не зарегистрируется или не
   * выйдет
   *
